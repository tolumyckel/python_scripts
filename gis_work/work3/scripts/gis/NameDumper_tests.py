...
def test_dumpFileList():
    ...
    filename = os.listdir(os.path.join(path,"TestFolder"))
    with open('dumpfile.txt','w') as dumpfile:
        for name in filename:
            if ".txt" in name:
                dumpfile.writelines(name)
    desc = ""
    expected = ["one.txt","two.txt"]
    #actual = FileUtils.getFileContent("dumpfile.txt")
    actual = NameDumper.dumpFileList("TestFolder","dumpfile.txt")
    print_test_results(NameDumper.dumpFileList, desc, expected, actual)
